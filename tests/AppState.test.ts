...
describe('AppState', () => {
  ...
  it('starts with null last server and thread IDs', () => {
    expect(state.lastServerId.value).toBeNull();
    expect(state.lastThreadId.value).toBeNull();
  });

  it('can set last server ID', () => {
    state.setLastServerId('server-1');
    expect(state.lastServerId.value).toBe('server-1');
  });

  it('can set last thread ID', () => {
    state.setLastThreadId('thread-1');
    expect(state.lastThreadId.value).toBe('thread-1');
  });
